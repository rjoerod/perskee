...
import { TASK_LIST } from "../../../util/mysql";
...
const EpicGenerateTasksButton = ({
	...
	const onClick = async () => {
		...
		try {
			...
			// mutate(`/api/task/getByEpic/${epic.id}`);
			setOpen(false);
		} catch (e) {
...
const useLists = () => {
	// TODO: Retrieve indexed data
	// const { data } = useSWR(`/api/list/all/`, fetcher);
	const data = { lists: [] as any };

	// limit to lists that are non-epic
	return data?.lists?.filter((list: List) => list.board_id == 1);
};

const EpicGenerateTasksButtonQuery = ({
	epic,
	boardId,
}: {
	epic: Task;
	...
	return (
		<EpicGenerateTasksButton epic={epic} boardId={boardId} lists={lists} />
	);
};
