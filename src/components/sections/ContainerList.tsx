...
function getListsWithChanges(
	...
	const changedListsBetween = currArray.filter((curr, idx) => {
		const prev = prevArray.find((prev) => prev[0] == curr[0]) ?? prevArray[idx];
...
const updateTaskOrder = async (
	prev: Record<string, Task[]> | null,
	curr: Record<string, Task[]> | null,
	listData: List[],
	boardId: number,
	allItems: Task[]
) => {
	...
	// TODO: replace with request for new local data
...
function ContainerList({
	...
	const [items, setItems] = useState<Record<string, Task[]>>(savedItems);
	...
	const itemsBeforeDrag = useRef<null | Record<string, Task[]>>(null);
	...
	const handleDragEnd = useCallback(
		async ({ active, over }: DragEndEvent) => {
			// Order List
			if (
				active?.data?.current?.sortable.containerId === "list-sorting-context"
			) {
	...
	return (
		...
		>
			...
			<div className="w-72">
				<div className="w-72">
					<AddListButton boardId={boardId} nextSortedOrder={nextSortedOrder} />
				</div>
			</div>
		</DndContext>
	);
